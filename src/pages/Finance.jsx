...
export default function Finance({ dark }) {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(id)) {
      ...
      tag.id = id; tag.innerHTML = FINANCE_CSS;
  ...
  return (
    <div className={`abk-fin abk-texture${dark?' abk-dark':''}`} style={{ background:'var(--cream)', minHeight:'100vh', position:'relative', transition:'background .3s' }}>
      ...
      <div style={{ position:'relative', zIndex:1, padding:'1.5rem 1.5rem 3rem' }}>
        ...
        {/* ── Period Selector ───────────────────────────────────────────── */}
        <div className="abk-anim-fade-in" style={{ display:'flex', alignItems:'center', gap:8, marginBottom:'1.1rem', animationDelay:'.08s' }}>
        ...
        {/* ── KPI Cards ─────────────────────────────────────────────────── */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,minmax(0,1fr))', gap:10, marginBottom:'1.1rem' }}>
        ...
        {activeTab === 'overview' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            ...
            <div className="abk-anim-fade-up" style={{ gridColumn:'1 / -1', background:'var(--card)', border:'1px solid var(--border)', borderRadius:16, overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,.06)', animationDelay:'.24s', transition:'background .3s, border-color .3s' }}>
              <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border-light)', background:'var(--cream-deep)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        {activeTab === 'expenses' && (
          <>
            {/* Filters */}
            <div className="abk-anim-fade-in" style={{ display:'flex', gap:8, marginBottom:'1rem', animationDelay:'.06s' }}>
            ...
            {/* Expense table */}
            <div className="abk-anim-scale-in" style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:16, overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,.06)', animationDelay:'.12s', transition:'background .3s, border-color .3s' }}>
